...
"""
Physics of Ultrasound Beamforming Simulation: Frequency Sweep for Synchronous Beamforming

This script simulates ultrasound beamforming with a frequency sweep from 140 kHz to 220 kHz,
emulating a synchronous ultrasound transducer. In synchronous beamforming all elements are excited
with the same phase (i.e. no delay correction for focusing is applied). The simulation computes the
complex summed field using delay-and-sum beamforming, applies apodization (Hanning window), models
geometric spreading and frequency-dependent attenuation, and then coherently sums the complex fields.

The composite pressure field is obtained by taking the real part of the arithmetic mean of the
normalized complex fields across frequencies. This preserves both constructive and destructive
...
for f in freqs:
    ...
    # For synchronous beamforming, do not apply a focusing delay.
    complex_field = np.exp(1j * phase_const_current * distance_to_point)
...
fig, axs = plot_panels(
    ...
    titles=[
        "Composite Synchronous Pressure Field (Coherent Sum)\n(Frequency range: 140-220 kHz)",
        "Composite Synchronous Pressure Field (RSS Sum)\n(Frequency range: 140-220 kHz)"
    ],
...
export_filename = os.path.join(export_dir, "wavephasesyncsweep.png")
